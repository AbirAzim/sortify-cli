...
Works on **macOS, Linux, and Windows** (PowerShell, cmd.exe, Git Bash, WSL) —
requires only Node.js. Folder names generated by `combine`/`split`/`suggest`
are automatically made filesystem-safe on all three (Windows-reserved
characters and device names like `CON`/`COM1` are stripped/renamed), and the
progress bar and spinner fall back to plain ASCII on a classic Windows
console that doesn't render Unicode well (Windows Terminal renders it fine).
